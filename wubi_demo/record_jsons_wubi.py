...
def json_letter(text):
    ...
    jieba_sentence = jieba_sentence.replace('、','，').replace('， ','，').replace('： ','：').replace(' ','')
    ...
    wubi_list = wubi(jieba_sentence)
    letters = ''
    for wubi in wubi_list:
        wubi = ''.join(wubi)
        wubi = wubi.replace('ntyc','ntcy')
        wubi = wubi + ' '
        letters += wubi
    print (letters)
...
for i in range(0, len(page_list)):
    ...
    ppt_name = './scripts_wubi/' + ppt_name
